...
默认启用静止帧复用：字幕进出场逐帧渲染，稳定停留期间复用包含人物、背景和字幕的完整 JPEG。
帧序列通过硬链接展开后交给项目自带 FFmpeg 编码，最终仍为恒定 60fps；分块只含视频。
音频由 Python 调用 FFmpeg，按时间轴毫秒位置（含 1 秒片头）一次性流式组装为 48kHz 双声道 PCM，
自动补静音、裁切超长音频并处理 MP3 编码延迟，最终只编码一次 AAC，不再逐块调用 Remotion 输出 WAV。
当前直接音轨路径支持 Director 的非重叠对白；重叠音轨会报错，不会静默丢弃，需使用旧路径。
带缩放或转场的背景保守地逐帧处理。每块日志记录实际浏览器帧数以及画面、清理、编码耗时；
完整音轨的组装耗时单独记录为 `timeline audio`。
需要旧路径对照时加 `--no-frame-reuse`。复用模式改变渲染签名，旧分块不会混入新输出。
新增持续动画时需同时更新 `src/lib/static-frame-plan.ts` 的动态区间判断，或暂时使用旧路径。
