...
日语角色配音默认依赖 Qwen3-TTS，也可切换到 Fish Speech。`scripts/synthesize_script.py` 默认调用本地 Qwen3-TTS 服务；需要使用 Fish Speech 时传 `--tts-engine fishspeech`。中文配音或跨语种克隆路线依赖 IndexTTS，后续可用于“日语克隆说中文”的版本。
...
旧内容可以直接预览或重渲染；如果希望把生成文件迁移成上述结构化字段，再重新运行一次 `director.py build --overwrite`。
...
- `subtitleJa`、`subtitleZh`、`speakerLabel` 等字幕内容或 timeline 结构变了：重新跑 `director.py build --overwrite`；只有 `spokenText` 或音频也变更时才需要重跑 TTS。
